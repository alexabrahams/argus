...
import argus._compression as c
...
def get_random_df(nrows, ncols):
    ret_df = pd.DataFrame(
        ...
        columns=["".join([chr(random.randint(ord("A"), ord("Z"))) for _ in range(8)]) for _ in range(ncols)],
    )
...
def main():
    ...
    for df_length in (1000, 3000, 10000, 30000):
        for n_chunks in (1, 2, 4, 8, 16, 32, 64, 128):
            print(
                "\n\n----------- High compression: {}, Chunks: {}, DataFrame size: {} ------------".format(
                    use_HC, n_chunks, df_length
                )
            )
