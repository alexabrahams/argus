...
def get_random_df(num_chunks):
    ...
    data_to_write = create_test_data(
        size=25000,
        index=True,
        multiindex=False,
        random_data=True,
...
def run_scenario(result_text, rounds, num_requests, num_chunks, parallel_lz4, use_async, async_argus_pool_workers=None):
    ...
    print(
        ...
        "parallel_lz4={}, async_argus_pool_workers={}: {}".format(
            result_text,
            use_async,
            num_chunks,
            num_requests,
...
def main():
    ...
    n_async_argus_pool_workers = (2, 4, 8)

    for num_chunks in n_num_chunks:
        for use_async in n_use_async:
            for async_argus_pool_workers in n_async_argus_pool_workers if use_async else (4,):
                    ...
                    for num_requests in n_num_requests:
                        for rounds in n_rounds:
                            run_scenario(
                                ...
                                async_argus_pool_workers=async_argus_pool_workers,
                            )
...
# Experiment results: async=False, chunks/write=2, writes/round=64, rounds=2, parallel_lz4=False, async_argus_pool_workers=4: ['10.109', '0.000', '10.109', '10.109']
# Experiment results: async=True, chunks/write=2, writes/round=64, rounds=2, parallel_lz4=False, async_argus_pool_workers=2: ['7.169', '0.000', '7.169', '7.169']
# Experiment results: async=True, chunks/write=2, writes/round=64, rounds=2, parallel_lz4=False, async_argus_pool_workers=4: ['5.327', '0.000', '5.327', '5.327']
# Experiment results: async=True, chunks/write=2, writes/round=64, rounds=2, parallel_lz4=False, async_argus_pool_workers=8: ['5.410', '0.000', '5.410', '5.410']
