...
from argus._config import FwPointersCfg
...
def gen_oneminute_dataset(n_row, n_col, dense):
    ...
    timestamps.extend(
        list(
            rrule(MINUTELY, count=n_row % active_minutes_daily, dtstart=dt(random.randrange(2006, 2016), 1, 1)),
        )
    )
...
def insert_random_data(config, args, n_rows):
    store = Argus(args.mongodb, app_name="benchmark")
