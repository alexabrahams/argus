...
from ..hosts import get_argus_lib
...
def copy_symbols_helper(src, dest, log, force, splice):
    def _copy_symbol(symbols):
        for symbol in symbols:
            with ArgusTransaction(dest, symbol, USER, log) as mt:
                ...
                if existing_data:
                    if force:
                        ...
                        logger.warn(
                            "Symbol: {} already exists in {}@{}, use --force to overwrite or --splice to join "
                            "with existing data".format(symbol, _get_host(dest).get("l"), _get_host(dest).get("mhost"))
                        )
                        continue
                ...
                if existing_data and splice:
                    ...
                    preserve_start = to_pandas_closed_closed(
                        DateRange(None, new_data.index[0].to_pydatetime(), interval=CLOSED_OPEN)
                    ).end
                    preserve_end = to_pandas_closed_closed(
                        DateRange(new_data.index[-1].to_pydatetime(), None, interval=OPEN_CLOSED)
                    ).start
...
def main():
    ...
    Example:
        argus_copy_data --log "Copying data" --src user.library@host1 --dest user.library@host2 symbol1 symbol2
    """
    setup_logging()
    ...
    p.add_argument(
        "--splice", default=False, action="store_true", help="Keep existing data before and after the new data."
    )
    ...
    src = get_argus_lib(opts.src)
    dest = get_argus_lib(opts.dest)
