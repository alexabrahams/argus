...
from ..exceptions import AsyncArgusException
...
class AsyncArgus(LazySingletonTasksCoordinator):
    _instance = None
    ...
    def __init__(self, pool_size):
        ...
        if not type(self)._SINGLETON_LOCK._is_owned():
            raise AsyncArgusException("AsyncArgus is a singleton, can't create a new instance")

        # Enforce the singleton pattern
        with type(self)._SINGLETON_LOCK:
            super(AsyncArgus, self).__init__(pool_size)
    ...
    def _verify_request(store, is_modifier, **kwargs):
        library_name = None if store is None else store._argus_lib.get_name()
    ...
    def _schedule_request(self, request):
        try:
            new_id, new_future = self.submit_task(False, _argus_task_exec, request)
            request.id = new_id
    ...
    def submit_argus_request(self, store, fun, is_modifier, *args, **kwargs):
        lib_name, symbol, kind, callback, mongo_retry = AsyncArgus._verify_request(store, is_modifier, **kwargs)
        ...
        with type(self)._POOL_LOCK:  # class level lock, since it is a Singleton
            if self.local_shutdown:
                raise AsyncArgusException("AsyncArgus has been shutdown and can no longer accept new requests.")
    ...
    def _request_finished(self, request):
        with type(self)._POOL_LOCK:
            ...
            if self.deferred_requests:
                ...
                super(AsyncArgus, self).shutdown()
    ...
    def reset(self, pool_size=None, timeout=None):
        ...
        super(AsyncArgus, self).reset(pool_size, timeout)
        with type(self)._SINGLETON_LOCK:
            self.requests_per_library = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
            self.requests_by_id = dict()
            self.local_shutdown = False
    ...
    def shutdown(self, timeout=None):
        ...
        with type(self)._POOL_LOCK:
            ...
            if self.total_pending_requests() == 0:
                ...
                super(AsyncArgus, self).shutdown(timeout=timeout)

    def await_termination(self, timeout=None):
        while self.total_pending_requests() > 0:
            AsyncArgus.wait_requests(
                self.requests_by_id.values() + self.deferred_requests, do_raise=False, timeout=timeout
            )
        # super(AsyncArgus, self).await_termination(timeout)
    ...
    def wait_request(request, do_raise=False, timeout=None):
        if request is None:
            return
        if not AsyncArgus._wait_until_scheduled((request,), timeout):
            raise AsyncArgusException("Timed-out while waiting for request to be scheduled")
        while request.is_completed:
            AsyncArgus.wait_tasks((request.future,), timeout=timeout, raise_exceptions=do_raise)

    @staticmethod
    def wait_requests(requests, do_raise=False, timeout=None):
        if not AsyncArgus._wait_until_scheduled(requests, timeout):
            raise AsyncArgusException("Timed-out while waiting for request to be scheduled")
        while requests and not all(r.is_completed for r in requests):
            AsyncArgus.wait_tasks(
                tuple(r.future for r in requests if not r.is_completed and r.future is not None),
                timeout=timeout,
    ...
    def filter_finished_requests(requests, do_raise=True):
        ...
        if do_raise:
            AsyncArgus.raise_errored(done_requests)
...
ASYNC_ARGUS = AsyncArgus.get_instance()
async_argus_submit = ASYNC_ARGUS.submit_argus_request
async_wait_request = ASYNC_ARGUS.wait_request
async_wait_requests = ASYNC_ARGUS.wait_requests
async_shutdown = ASYNC_ARGUS.shutdown
...
#         return async_argus_submit(self, func, True, *args, **kwargs)
#
#     return wrapper
#
#
...
#     @wraps(func)
#     def wrapper(self, *args, **kwargs):
#         return async_argus_submit(self, func, False, *args, **kwargs)
#
#     return wrapper
