...
def enable_sharding(argus, library_name, hashed=True, key="symbol"):
    """
    Enable sharding on a library

    Parameters:
    -----------
    argus: `argus.Argus` Argus class
    ...
    key: `basestring` key to be used for sharding. Defaults to 'symbol', applicable to
         all of Argus's built-in stores except for BSONStore, which typically uses '_id'.
         See https://docs.mongodb.com/manual/core/sharding-shard-key/ for details.
    """
    c = argus._conn
    lib = argus[library_name]._argus_lib
    dbname = lib._db.name
