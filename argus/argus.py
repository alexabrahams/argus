...
from .exceptions import LibraryNotFoundException, ArgusException, QuotaExceededException
from .hooks import get_mongodb_uri
from .store import version_store, bson_store, metadata_store
from .tickstore import tickstore, toplevel

__all__ = ["Argus", "VERSION_STORE", "METADATA_STORE", "TICK_STORE", "CHUNK_STORE", "register_library_type"]

# Set default logging handler to avoid "No handler found" warnings.
logger = logging.getLogger(__name__)
logger.addHandler(logging.NullHandler())

# Default Argus application name: 'argus'
...
class Argus(object):
    ...
    accessible in Mongo.
    Each database contains one or more ArgusLibrarys which may have implementation
    specific functionality.

    Current Mongo Library types:
       - argus.VERSION_STORE - Versioned store for chunked Pandas and numpy objects
                                (other Python types are pickled)
       - argus.TICK_STORE - Tick specific library. Supports 'snapshots', efficiently
                             stores updates, not versioned.
       - argus.METADATA_STORE - Stores metadata with timestamps

    Argus and ArgusLibrary are responsible for Connection setup, authentication,
    dispatch to the appropriate library implementation, and quotas.
    """

    DB_PREFIX = "argus"
    METADATA_COLL = "ARGUS"
    METADATA_DOC_ID = "ARGUS_META"
    ...
    def __setstate__(self, state):
        return Argus.__init__(self, **state)
    ...
    def _list_libraries_cached(self, newer_than_secs=None):
        ...
        Returns
        -------
        List of Argus library names from a cached collection (global per mongo cluster) in mongo.
    ...
    def library_exists(self, library):
        ...
        try:
            ...
            ArgusLibraryBinding(self, library).get_library_type()
    ...
    def initialize_library(self, library, lib_type=VERSION_STORE, **kwargs):
        """
        Create an Argus Library or a particular type.
        ...
        lib_type : `str`
            The type of the library.  e.g. argus.VERSION_STORE or argus.TICK_STORE
            Or any type registered with register_library_type
            Default: argus.VERSION_STORE
    ...
    def delete_library(self, library):
        """
        Delete an Argus Library, and all associated collections in the MongoDB.
    ...
    def __getitem__(self, key):
        if isinstance(key, string_types):
            ...
            raise ArgusException("Unrecognised library specification - use [libraryName]")
    ...
    def set_quota(self, library, quota):
        ...
        """
        ArgusLibraryBinding(self, library).set_quota(quota)
    ...
    def get_quota(self, library):
        ...
        library : `str`
            The name of the library. e.g. 'library' or 'user.library'
        """
        return ArgusLibraryBinding(self, library).get_quota()
    ...
    def check_quota(self, library):
        ...
        Raises
        ------
        argus.exceptions.QuotaExceededException if the quota has been exceeded
        """
        ArgusLibraryBinding(self, library).check_quota()
...
class ArgusLibraryBinding(object):
    ...
    def __init__(self, argus, library):
        self.argus = argus
        self._curr_conn = self.argus._conn
        self._lock = threading.RLock()
        database_name, library = self._parse_db_lib(library)
        self.library = library
        self.database_name = database_name
        self._auth(self.argus._conn[self.database_name])

    @property
    def _db(self):
        with self._lock:
            argus_conn = self.argus._conn
            if argus_conn is not self._curr_conn:
                self._auth(argus_conn[self.database_name])  # trigger re-authentication if Argus has been reset
                self._curr_conn = argus_conn
        return self.argus._conn[self.database_name]
    ...
    def __str__(self):
        return """<ArgusLibrary at %s, %s.%s>
%s""" % (
            hex(id(self)),
            self._db.name,
            self._library_coll.name,
            indent(str(self.argus), 4),
        )
    ...
    def __getstate__(self):
        return {"argus": self.argus, "library": ".".join([self.database_name, self.library])}

    def __setstate__(self, state):
        return ArgusLibraryBinding.__init__(self, state["argus"], state["library"])

    @mongo_retry
    def _auth(self, database):
        # Get .mongopass details here
        if not hasattr(self.argus, "mongo_host"):
            return

        auth = get_auth(self.argus.mongo_host, self.argus._application_name, database.name)
        if auth:
    ...
    def set_quota(self, quota_bytes):
        ...
        """
        self.set_library_metadata(ArgusLibraryBinding.QUOTA, quota_bytes)
    ...
    def get_quota(self):
        ...
        """
        return self.get_library_metadata(ArgusLibraryBinding.QUOTA)
    ...
    def check_quota(self):
        ...
        self.quota = self.get_library_metadata(ArgusLibraryBinding.QUOTA)
        if self.quota is None or self.quota == 0:
            self.quota = 0
            return

        # Figure out whether the user has exceeded their quota
        library = self.argus[self.get_name()]
        stats = library.stats()
    ...
    def set_library_type(self, lib_type):
        self.set_library_metadata(ArgusLibraryBinding.TYPE_FIELD, lib_type)

    @mongo_retry
    def get_library_metadata(self, field):
        lib_metadata = self._library_coll[self.argus.METADATA_COLL].find_one({"_id": self.argus.METADATA_DOC_ID})
    ...
    def set_library_metadata(self, field, value):
        self._library_coll[self.argus.METADATA_COLL].update_one(
            {"_id": self.argus.METADATA_DOC_ID}, {"$set": {field: value}}, upsert=True
        )
