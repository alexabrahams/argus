...
class TopLevelTickStore(object):
    ...
    def _add_libraries(self):
        ...
        libraries = [lib for lib in self._argus_lib.argus.list_libraries() if regex.search(lib)]
    ...
    def add(self, date_range, library_name):
        ...
        library_name: The name of the underlying library. This must be the name of a valid Argus library
        """
        # check that the library is valid
        try:
            self._argus_lib.argus[library_name]
        except Exception as e:
            logger.error("Could not load library")
            raise e
        assert date_range.start and date_range.end, "Date range should have start and end properties {}".format(
            date_range
        )
        start = (
...
overlapping libraries: {}""".format(
    ...
    def read(self, symbol, date_range, columns=None, include_images=False):
        ...
        for lib in libraries:
            try:
                df = lib.library.read(
                    symbol, lib.date_range.intersection(date_range), columns, include_images=include_images
                )
                dfs.append(df)
    ...
    def write(self, symbol, data):
        ...
        for res in cursor:
            library = self._argus_lib.argus[res["library_name"]]
    ...
    def _get_libraries(self, date_range):
        ...
        rtn = [TickStoreLibrary(self._argus_lib.argus[library.library], library.date_range) for library in libraries]
    ...
    def _get_library_metadata(self, date_range):
        ...
        cursor = self._collection.find(
            query, projection={"library_name": 1, "start": 1, "end": 1}, sort=[("start", pymongo.ASCENDING)]
        )
