...
class TickStore:
    ...
    def __init__(self, argus_lib, chunk_size=100000):
        """
        Parameters
        ----------
        argus_lib : ArgusLibraryBinding
            Argus Library
        chunk_size : int
...
{indent(str(self._argus_lib), 4)}"""
    ...
    def _assert_nonoverlapping_data(self, symbol, start, end):
        ...
        doc = self._collection.find_one(
            {SYMBOL: symbol, START: {"$lt": end}},
            projection={START: 1, END: 1, "_id": 0},
    ...
    def _to_bucket(ticks, symbol, initial_image):
        ...
        rowmask = dict([(k, Binary(lz4_compressHC(np.packbits(v).tostring()))) for k, v in iteritems(rowmask)])
        for k, v in iteritems(data):
            if k != "index":
                v = np.array(v)
                v = TickStore._ensure_supported_dtypes(v)
                rtn[COLUMNS][k] = {
                    DATA: Binary(lz4_compressHC(v.tostring())),
