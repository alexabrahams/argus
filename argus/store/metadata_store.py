...
class MetadataStore(BSONStore):
    ...
    def initialize_library(cls, argus_lib, hashed=True, **kwargs):
        MetadataStore(argus_lib)._ensure_index()
        BSONStore.initialize_library(argus_lib, hashed, **kwargs)

    @mongo_retry
    def _ensure_index(self):
        self.create_index(
            [("symbol", pymongo.ASCENDING), ("start_time", pymongo.DESCENDING)], unique=True, background=True
        )
    ...
    def _reset(self):
        self._collection = self._argus_lib.get_top_level_collection().metadata
    ...
    def read(self, symbol, as_of=None):
        ...
        if as_of is not None:
            res = self.find_one(
                {"symbol": symbol, "start_time": {"$lte": as_of}}, sort=[("start_time", pymongo.DESCENDING)]
            )
        else:
    ...
    def append(self, symbol, metadata, start_time=None):
        ...
        self.find_one_and_update(
            {"symbol": symbol}, {"$set": {"end_time": start_time}}, sort=[("start_time", pymongo.DESCENDING)]
        )
    ...
    def prepend(self, symbol, metadata, start_time=None):
        ...
        if old_metadata is not None:
            ...
            if old_metadata["metadata"] == metadata:
                self.find_one_and_update(
                    {"symbol": symbol}, {"$set": {"start_time": start_time}}, sort=[("start_time", pymongo.ASCENDING)]
                )
    ...
    def pop(self, symbol):
        ...
        mongo_retry(self.find_one_and_update)(
            {"symbol": symbol}, {"$unset": {"end_time": ""}}, sort=[("start_time", pymongo.DESCENDING)]
        )
