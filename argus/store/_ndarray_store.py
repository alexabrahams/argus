...
def _attempt_update_unchanged(symbol, unchanged_segment_ids, collection, version, previous_version):
    ...
    if unchanged_ids != matched_segments_ids:
        logger.error(
            "Mismatched unchanged segments for {}: {} != {} (query spec={})".format(
                symbol, unchanged_ids, matched_segments_ids, spec
            )
        )
        raise DataIntegrityException(
            "Symbol: {}:{} update_many updated {} segments instead of {}".format(
...
def _update_fw_pointers(collection, symbol, version, previous_version, is_append, shas_to_add=None):
    ...
    """
    version[FW_POINTERS_CONFIG_KEY] = ARGUS_FORWARD_POINTERS_CFG.name  # get the str as enum is not BSON serializable

    if ARGUS_FORWARD_POINTERS_CFG is FwPointersCfg.DISABLED:
        return
...
def _spec_fw_pointers_aware(symbol, version, from_index=None, to_index=None):
    ...
    # Version was written with an older argus, not FW-pointers aware, or written with FW pointers disabled
...
class NdarrayStore(object):
    ...
    def _ensure_index(collection):
        try:
            ...
            collection.create_index(
                [("symbol", pymongo.ASCENDING), ("parent", pymongo.ASCENDING), ("segment", pymongo.ASCENDING)],
                unique=True,
                background=True,
            )
            # Used for efficient SHA-based read queries that have index ranges
            collection.create_index(
                [("symbol", pymongo.ASCENDING), ("sha", pymongo.ASCENDING), ("segment", pymongo.ASCENDING)],
                unique=True,
                background=True,
            )
    ...
    def read(self, argus_lib, version, symbol, read_preference=None, **kwargs):
    ...
    def _do_read(self, collection, version, symbol, index_range=None):
        ...
        if segment_count is not None and i + 1 != segment_count:
            raise OperationFailure(
                "Incorrect number of segments returned for {}:{}.  Expected: {}, but got {}. {}".format(
    ...
    def append(self, argus_lib, version, symbol, item, previous_version, dtype=None, dirty_append=True):
        ...
        version[FW_POINTERS_CONFIG_KEY] = ARGUS_FORWARD_POINTERS_CFG.name
    ...
    def _do_append(self, collection, version, symbol, item, previous_version, dirty_append):
        data = item.tostring()
        # Compatibility with Argus 1.22.0 that didn't write base_sha into the version document
            ...
            if len(item) > 0:
                ...
                try:
                    ...
                    if ARGUS_FORWARD_POINTERS_CFG is FwPointersCfg.DISABLED:
                        ...
                        if ARGUS_FORWARD_POINTERS_CFG is FwPointersCfg.HYBRID:
                        ...
                        _update_fw_pointers(
                            collection, symbol, version, previous_version, is_append=True, shas_to_add=(sha,)
                        )
    ...
    def _concat_and_rewrite(self, collection, version, symbol, item, previous_version):
        ...
        if len(unchanged_segments) < previous_version["segment_count"] - previous_version["append_count"] - 1:
            raise DataIntegrityException(
                "Symbol: %s:%s expected %s segments but found %s"
                % (
                    symbol,
    ...
    def check_written(collection, symbol, version):
        ...
        if version.get(FW_POINTERS_CONFIG_KEY) == FwPointersCfg.HYBRID.name and ARGUS_FORWARD_POINTERS_RECONCILE:
            ...
            if seen_chunks != seen_chunks_reverse_pointers:
                raise pymongo.errors.OperationFailure(
                    "Failed to reconcile forward pointer chunks ({}). "
                    "Parent {}. "
    ...
    def write(self, argus_lib, version, symbol, item, previous_version, dtype=None):
        collection = argus_lib.get_top_level_collection()
        if item.dtype.hasobject:
            raise UnhandledDtypeException()
    ...
    def _do_write(self, collection, version, symbol, item, previous_version, segment_offset=0):
        ...
        if previous_version:
            symbol_all_previous_shas.update(
                Binary(x["sha"]) for x in collection.find({"symbol": symbol}, projection={"sha": 1, "_id": 0})
            )
        ...
        for i, chunk in zip(idxs, compressed_chunks):
            ...
            if ARGUS_FORWARD_POINTERS_CFG is FwPointersCfg.DISABLED:
                if sha not in symbol_all_previous_shas:
                    ...
                    bulk.append(
                        pymongo.UpdateOne(
                            segment_spec, {"$set": segment, "$addToSet": {"parent": version["_id"]}}, upsert=True
                        )
                    )
                else:
                ...
                elif ARGUS_FORWARD_POINTERS_CFG is FwPointersCfg.HYBRID:
        ...
        segment_index = self._segment_index(
            item, existing_index=existing_index, start=segment_offset, new_segments=segment_index
        )
        if segment_index:
            version["segment_index"] = segment_index
