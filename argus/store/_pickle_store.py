...
class PickleStore:
        ...
        if blob is not None:
            if blob == _MAGIC_CHUNKEDV2:
                ...
                if blob[: len(_MAGIC_CHUNKED)] == _MAGIC_CHUNKED:
                    logger.error(
                        "Data was written by unsupported version of pickle store for symbol %s. Upgrade Argus and try again"
                        % symbol
                    )
    ...
    def write(self, argus_lib, version, symbol, item, _previous_version):
        ...
        for seg, d in enumerate(data):
            ...
            collection.update_one(
                {"symbol": symbol, "sha": sha}, {"$set": segment, "$addToSet": {"parent": version["_id"]}}, upsert=True
            )
