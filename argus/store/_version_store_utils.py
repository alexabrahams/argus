...
from argus._config import FW_POINTERS_REFS_KEY, FW_POINTERS_CONFIG_KEY, FwPointersCfg
...
def cleanup(argus_lib, symbol, version_ids, versions_coll, shas_to_delete=None, pointers_cfgs=None):
    """
    ...
    # All the versions of the symbol have been created with old argus or with disabled forward pointers.
...
def analyze_symbol(instance, sym, from_ver, to_ver, do_reads=False):
    ...
    Parameters
    ----------
    instance : `argus.store.version_store.VersionStore`
...
def is_safe_to_append(instance, sym, input_v):
    ...
    input_v : `int` or `argus.store.version_store.VersionedItem`
    ...
    return not _fast_check_corruption(
        instance._collection, sym, input_v, check_count=True, check_last_segment=True, check_append_safe=True
    )
