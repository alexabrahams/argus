...
from ..exceptions import NoDataFoundException, DuplicateSnapshotException, ArgusException
...
_TYPE_HANDLERS = []
ARGUS_VERSION = None
ARGUS_VERSION_NUMERICAL = None


def register_version(version, numerical):
    global ARGUS_VERSION, ARGUS_VERSION_NUMERICAL
    ARGUS_VERSION = version
    ARGUS_VERSION_NUMERICAL = numerical
...
class VersionStore(object):
    ...
    def initialize_library(cls, argus_lib, hashed=True, **kwargs):
        c = argus_lib.get_top_level_collection()

        if "strict_write_handler" in kwargs:
            argus_lib.set_library_metadata("STRICT_WRITE_HANDLER_MATCH", bool(kwargs.pop("strict_write_handler")))

        for th in _TYPE_HANDLERS:
            th.initialize_library(argus_lib, **kwargs)
        VersionStore._bson_handler.initialize_library(argus_lib, **kwargs)
        VersionStore(argus_lib)._ensure_index()
    ...
    def _ensure_index(self):
        ...
        collection.versions.create_index(
            [("symbol", pymongo.ASCENDING), ("version", pymongo.DESCENDING)], unique=True, background=True
        )
        collection.versions.create_index(
            [("symbol", pymongo.ASCENDING), ("version", pymongo.DESCENDING), ("metadata.deleted", pymongo.ASCENDING)],
    ...
    def __init__(self, argus_lib):
        self._argus_lib = argus_lib
        # Do we allow reading from secondaries
        self._allow_secondary = self._argus_lib.argus._allow_secondary
        self._reset()
    ...
    def _with_strict_handler_match(self):
        if self._with_strict_handler is None:
            strict_meta = self._argus_lib.get_library_metadata("STRICT_WRITE_HANDLER_MATCH")
    ...
    def _reset(self):
        ...
        self._collection = self._argus_lib.get_top_level_collection()
    ...
    def __getstate__(self):
        return {"argus_lib": self._argus_lib}
...
%s""" % (
    ...
    def read(self, symbol, as_of=None, date_range=None, from_version=None, allow_secondary=None, **kwargs):
        ...
        as_of : `str` or `int` or `datetime.datetime`
            ...
            `str` : snapshot name which contains the version
            `datetime.datetime` : the version of the data that existed as_of the requested point in time
        date_range: `argus.date.DateRange`
        ...
        allow_secondary : `bool` or `None`
            ...
            `None` : use the settings from the top-level `Argus` object used to query this version store.
        ...
        try:
            ...
            return self._do_read(
                symbol, _version, from_version, date_range=date_range, read_preference=read_preference, **kwargs
            )
            ...
            return self._do_read_retry(
                symbol, _version, from_version, date_range=date_range, read_preference=ReadPreference.PRIMARY, **kwargs
            )
    ...
    def get_argus_version(self, symbol, as_of=None):
        """
        Return the numerical representation of the argus version used to write the last (or as_of) version for
        the given symbol.
        ...
        -------
        argus_version : int
            The numerical representation of Argus version, used to create the specified symbol version
        """
        return self._read_metadata(symbol, as_of=as_of).get("argus_version", 0)
    ...
    def _do_read(self, symbol, version, from_version=None, **kwargs):
        ...
        return VersionedItem(
            symbol=symbol,
            library=self._argus_lib.get_name(),
            version=version["version"],
            metadata=version.pop("metadata", None),
            data=data,
            host=self._argus_lib.argus.mongo_host,
        )
    ...
    def _read_metadata(self, symbol, as_of=None, read_preference=None):
        if read_preference is None:
            ...
            read_preference = (
                ReadPreference.PRIMARY_PREFERRED if not self._allow_secondary else ReadPreference.SECONDARY_PREFERRED
            )
        ...
        if as_of is None:
            ...
            _version = versions_coll.find_one(
                {"symbol": symbol, "_id": {"$lt": bson.ObjectId.from_datetime(as_of + timedelta(seconds=1))}},
    ...
    def append(self, symbol, data, metadata=None, prune_previous_version=True, upsert=True, **kwargs):
        ...
        """
        self._argus_lib.check_quota()
        version = {"_id": bson.ObjectId()}
        version["argus_version"] = ARGUS_VERSION_NUMERICAL
        ...
        if upsert and previous_version is None:
            return self.write(
                symbol=symbol, data=data, prune_previous_version=prune_previous_version, metadata=metadata
            )
        ...
        next_ver = self._version_nums.find_one_and_update(
            {
                "symbol": symbol,
            },
            {"$inc": {"version": 1}},
        ...
        if next_ver != previous_version["version"] + 1:
            dirty_append = True
            logger.debug(
                """version_nums is out of sync with previous version document.
    ...
    def write(self, symbol, data, metadata=None, prune_previous_version=True, **kwargs):
        ...
        version["argus_version"] = ARGUS_VERSION_NUMERICAL
        version["symbol"] = symbol
        version["version"] = self._version_nums.find_one_and_update(
            {"symbol": symbol}, {"$inc": {"version": 1}}, upsert=True, new=True
        )["version"]
        version["metadata"] = metadata

        previous_version = self._versions.find_one(
            {"symbol": symbol, "version": {"$lt": version["version"]}}, sort=[("version", pymongo.DESCENDING)]
        )

        handler = self._write_handler(version, symbol, data, **kwargs)
        handler.write(self._argus_lib, version, symbol, data, previous_version, **kwargs)

        if prune_previous_version and previous_version:
            self._prune_previous_versions(
                symbol, keep_mins=kwargs.get("keep_mins", 120), new_version_shas=version.get(FW_POINTERS_REFS_KEY)
            )
    ...
    def _add_new_version_using_reference(self, symbol, new_version, reference_version, prune_previous_version):
        ...
        if lastv_seqn != new_version["version"]:
            raise OperationFailure(
                "The symbol {} has been modified concurrently ({} != {})".format(
        ...
        if last_look is None or last_look.get("deleted"):
            ...
            raise OperationFailure(
                "Failed to write metadata for symbol %s. "
        ...
        return VersionedItem(
            ...
            metadata=new_version.get("metadata"),
            data=None,
            host=self._argus_lib.argus.mongo_host,
        )
    ...
    def write_metadata(self, symbol, metadata, prune_previous_version=True, **kwargs):
        ...
        try:
            ...
            return self.write(
                symbol, data=None, metadata=metadata, prune_previous_version=prune_previous_version, **kwargs
            )

        # Reaching here means that and/or metadata exist and we are set to update the metadata
        new_version_num = self._version_nums.find_one_and_update(
            {"symbol": symbol}, {"$inc": {"version": 1}}, upsert=True, new=True
        )["version"]
    ...
    def restore_version(self, symbol, as_of, prune_previous_version=True):
        ...
        new_item = self.write(
            symbol, data=item.data, metadata=item.metadata, prune_previous_version=prune_previous_version
        )
        return new_item
    ...
    def _find_prunable_version_ids(self, symbol, keep_mins):
        ...
        return {
            v["_id"]: ([bson.binary.Binary(x) for x in v.get(FW_POINTERS_REFS_KEY, [])], get_fwptr_config(v))
            for v in cursor
        }
    ...
    def _write_audit(self, user, message, changed_version):
        ...
        mongo_retry(self._versions.update_many)(
            {"symbol": changed_version.symbol, "version": {"$in": [orig_version, new_version]}},
    ...
    def _cleanup_orphaned_chunks(self, dry_run):
        ...
        for symbol in chunks_coll.distinct("symbol"):
            ...
            if not dry_run:
                ...
                cleanup(lib._argus_lib, symbol, leaked_versions, versions_coll)
    ...
    def _cleanup_orphaned_versions(self, dry_run):
        ...
        # Using aggregate so we can unwind, and pull out 'parent', where 'parent' is older than a day.
        parents = versions_coll.aggregate(
            [
                {"$project": {"parent": True}},
                {"$unwind": "$parent"},
                {"$match": {"parent": parent_id_constraint}},
