...
class PandasSerializer(object):
    ...
    def _to_records(self, df, string_max_len=None, forced_dtype=None):
        ...
        if "" in columns:
            raise ArgusException("Cannot use empty string as a column name.")
        ...
        for arr, name in zip(ix_vals + column_vals, index_names + columns):
            arrays.append(
                _to_primitive(arr, string_max_len, forced_dtype=None if forced_dtype is None else forced_dtype[name])
            )

        if forced_dtype is None:
            dtype = np.dtype(
                [
                    (str(x), v.dtype) if len(v.shape) == 1 else (str(x), v.dtype, v.shape[1])
...
class DataFrameSerializer(PandasSerializer):
    ...
    def deserialize(self, item, force_bytes_to_unicode=False):
        ...
        if force_bytes_to_unicode:
            ...
            # of people migrating to py3. # https://github.com/manahl/argus/issues/598
