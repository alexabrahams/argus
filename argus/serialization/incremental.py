...
from argus._config import ARGUS_AUTO_EXPAND_CHUNK_SIZE
from argus.serialization.numpy_records import PandasSerializer
from .._compression import compress
from .._config import MAX_DOCUMENT_SIZE
from .._util import NP_OBJECT_DTYPE
from ..exceptions import ArgusSerializationException
...
class IncrementalPandasToRecArraySerializer(LazyIncrementalSerializer):
    def __init__(self, serializer, input_data, chunk_size, string_max_len=None):
        ...
        if not isinstance(input_data, (pd.DataFrame, pd.Series)):
            raise ArgusSerializationException(
                "IncrementalPandasToRecArraySerializer requires a pandas DataFrame or " "Series as data source input."
            )
    ...
    def _lazy_init(self):
        ...
        with self._lock:
            ...
            if len(self) > 0 and self.chunk_size > 1:
                rows_per_chunk = IncrementalPandasToRecArraySerializer._calculate_rows_per_chunk(
    ...
    def _calculate_rows_per_chunk(max_chunk_size, chunk):
        ...
        if rows_per_chunk < 1 and ARGUS_AUTO_EXPAND_CHUNK_SIZE:
        ...
        if rows_per_chunk < 1:
            raise ArgusSerializationException("Serialization failed to split data into max sized chunks.")
