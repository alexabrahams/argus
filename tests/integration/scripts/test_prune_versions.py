...
from argus.scripts import argus_prune_versions as mpv
from ...util import run_as_main


def test_prune_versions_symbol(mongo_host, library, library_name):
    with patch("argus.scripts.argus_prune_versions.prune_versions", autospec=True) as prune_versions, patch(
        "argus.scripts.utils.get_auth", return_value=Credential("admin", "adminuser", "adminpwd")
...
def test_prune_versions_full(mongo_host, library, library_name):
    with patch("argus.scripts.argus_prune_versions.do_db_auth", return_value=True):
...
def test_fix_broken_snapshot_references(library):
    library.write("cherry", "blob")
    one_day_ago = time.time() - (3600 * 24.0) - 10  # make sure we are a few seconds before 24 hours
