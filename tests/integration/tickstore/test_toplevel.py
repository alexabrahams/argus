...
from argus.date import DateRange, mktz
from argus.exceptions import NoDataFoundException, LibraryNotFoundException, OverlappingDataException
from argus.tickstore import tickstore
from argus.tickstore import toplevel

FEED_2010_LEVEL1 = toplevel.TickStoreLibrary(
    "FEED_2010.LEVEL1", DateRange(dt(2010, 1, 1), dt(2010, 12, 31, 23, 59, 59))
)
FEED_2011_LEVEL1 = toplevel.TickStoreLibrary(
    "FEED_2011.LEVEL1", DateRange(dt(2011, 1, 1), dt(2011, 12, 31, 23, 59, 59))
)
FEED_2012_LEVEL1 = toplevel.TickStoreLibrary(
    "FEED_2012.LEVEL1", DateRange(dt(2012, 1, 1), dt(2012, 12, 31, 23, 59, 59))
)
...
def test_should_return_libraries_for_the_given_daterange(toplevel_tickstore, start, end, expected):
    toplevel_tickstore._collection.insert_one(
        {"start": dt(2010, 1, 1), "end": dt(2010, 12, 31, 23, 59, 59), "library_name": "FEED_2010.LEVEL1"}
    )
    toplevel_tickstore._collection.insert_one(
        {"start": dt(2011, 1, 1), "end": dt(2011, 12, 31, 23, 59, 59), "library_name": "FEED_2011.LEVEL1"}
    )
    toplevel_tickstore._collection.insert_one(
        {"start": dt(2012, 1, 1), "end": dt(2012, 12, 31, 23, 59, 59), "library_name": "FEED_2012.LEVEL1"}
    )
...
def test_should_return_data_when_date_range_falls_in_a_single_underlying_library(toplevel_tickstore, argus):
    argus.initialize_library("FEED_2010.LEVEL1", tickstore.TICK_STORE_TYPE)
    tstore = argus["FEED_2010.LEVEL1"]
    argus.initialize_library("test_current.toplevel_tickstore", tickstore.TICK_STORE_TYPE)
    tickstore_current = argus["test_current.toplevel_tickstore"]
    toplevel_tickstore._collection.insert_one(
        {"start": dt(2010, 1, 1), "end": dt(2010, 12, 31, 23, 59, 59), "library_name": "FEED_2010.LEVEL1"}
    )
...
def test_should_return_data_when_date_range_spans_libraries(toplevel_tickstore, argus):
    argus.initialize_library("FEED_2010.LEVEL1", tickstore.TICK_STORE_TYPE)
    argus.initialize_library("FEED_2011.LEVEL1", tickstore.TICK_STORE_TYPE)
    tickstore_2010 = argus["FEED_2010.LEVEL1"]
    tickstore_2011 = argus["FEED_2011.LEVEL1"]
    toplevel_tickstore.add(
        DateRange(start=dt(2010, 1, 1), end=dt(2010, 12, 31, 23, 59, 59, 999000)), "FEED_2010.LEVEL1"
    )
    toplevel_tickstore.add(
        DateRange(start=dt(2011, 1, 1), end=dt(2011, 12, 31, 23, 59, 59, 999000)), "FEED_2011.LEVEL1"
    )
    dates = pd.date_range("20100101", periods=6, tz=mktz("Europe/London"))
...
def test_should_return_data_when_date_range_spans_libraries_even_if_one_returns_nothing(toplevel_tickstore, argus):
    argus.initialize_library("FEED_2010.LEVEL1", tickstore.TICK_STORE_TYPE)
    argus.initialize_library("FEED_2011.LEVEL1", tickstore.TICK_STORE_TYPE)
    tickstore_2010 = argus["FEED_2010.LEVEL1"]
    tickstore_2011 = argus["FEED_2011.LEVEL1"]
...
def test_should_add_underlying_library_where_none_exists(toplevel_tickstore, argus):
    argus.initialize_library("FEED_2010.LEVEL1", tickstore.TICK_STORE_TYPE)
    toplevel_tickstore.add(
        DateRange(start=dt(2010, 1, 1), end=dt(2010, 12, 31, 23, 59, 59, 999000)), "FEED_2010.LEVEL1"
    )
...
def test_should_add_underlying_library_where_another_library_exists_in_a_non_overlapping_daterange(
    toplevel_tickstore, argus
):
    toplevel_tickstore._collection.insert_one(
        {"library_name": "FEED_2011.LEVEL1", "start": dt(2011, 1, 1), "end": dt(2011, 12, 31)}
    )
    argus.initialize_library("FEED_2010.LEVEL1", tickstore.TICK_STORE_TYPE)
    toplevel_tickstore.add(
        DateRange(start=dt(2010, 1, 1), end=dt(2010, 12, 31, 23, 59, 59, 999000)), "FEED_2010.LEVEL1"
    )
    assert set([res["library_name"] for res in toplevel_tickstore._collection.find()]) == set(
        ["FEED_2010.LEVEL1", "FEED_2011.LEVEL1"]
    )
...
def test_should_raise_exception_if_date_range_for_library_overlaps_with_existing_libraries(toplevel_tickstore, argus):
    toplevel_tickstore._collection.insert_one(
        {"library_name": "FEED_2010.LEVEL1", "start": dt(2010, 1, 1), "end": dt(2010, 6, 30)}
    )
    argus.initialize_library("FEED_2010a.LEVEL1", tickstore.TICK_STORE_TYPE)
    with pytest.raises(OverlappingDataException) as e:
        toplevel_tickstore.add(
            DateRange(start=dt(2010, 6, 1), end=dt(2010, 12, 31, 23, 59, 59, 999000)), "FEED_2010a.LEVEL1"
        )
        assert toplevel_tickstore._collection.find_one({"library_name": "FEED_2010.LEVEL1"})
...
def test_should_successfully_do_a_roundtrip_write_and_read_spanning_multiple_underlying_libraries(
    ...
    lib2010 = argus["FEED_2010.LEVEL1"]
    ...
    lib2011 = argus["FEED_2011.LEVEL1"]
...
def test_should_list_symbols_from_the_underlying_library(toplevel_tickstore, argus, start, end, startr, endr):
    argus.initialize_library("FEED_2010.LEVEL1", tickstore.TICK_STORE_TYPE)
    argus.initialize_library("FEED_2011.LEVEL1", tickstore.TICK_STORE_TYPE)
    toplevel_tickstore.add(
        DateRange(start=dt(2010, 1, 1), end=dt(2010, 12, 31, 23, 59, 59, 999000)), "FEED_2010.LEVEL1"
...
def test_should_add_underlying_libraries_when_intialized(argus):
    argus.initialize_library("FEED_2010.LEVEL1", tickstore.TICK_STORE_TYPE)
    argus.initialize_library("FEED_2011.LEVEL1", tickstore.TICK_STORE_TYPE)
    argus.initialize_library("FEED.LEVEL1", toplevel.TICK_STORE_TYPE)
    toplevel_tickstore = argus["FEED.LEVEL1"]
    ...
    expected_results = {
        "FEED_2010.LEVEL1": {"start": dt(2010, 1, 1), "end": dt(2010, 12, 31, 23, 59, 59, 999000)},
        "FEED_2011.LEVEL1": {"start": dt(2011, 1, 1), "end": dt(2011, 12, 31, 23, 59, 59, 999000)},
    }
    assert expected_results == results


def test_should_write_top_level_with_list_of_dicts(argus):
    argus.initialize_library("FEED_2010.LEVEL1", tickstore.TICK_STORE_TYPE)
    argus.initialize_library("FEED_2011.LEVEL1", tickstore.TICK_STORE_TYPE)
    argus.initialize_library("FEED.LEVEL1", toplevel.TICK_STORE_TYPE)
    toplevel_tickstore = argus["FEED.LEVEL1"]
...
def test_should_write_top_level_with_correct_timezone(argus):
    ...
    toplevel_tickstore = argus["FEED.LEVEL1"]
    dates = pd.date_range(
        "20101230220000", periods=10, tz=mktz("America/New_York")
    )  # 10pm New York time is 3am next day UTC
    data = [{"index": dates[i], "a": i} for i in range(len(dates))]
    ...
    res = toplevel_tickstore.read(
        "blah", DateRange(start=dt(2010, 1, 1), end=dt(2011, 12, 31)), columns=list("a")
    ).tz_convert(utc)
...
def test_no_min_max_date(argus):
    argus.initialize_library("FEED_2010.LEVEL1", tickstore.TICK_STORE_TYPE)
    tstore = argus["FEED_2010.LEVEL1"]
    dates = pd.date_range("20100101", periods=6, tz=mktz("Europe/London"))
    df = pd.DataFrame(np.random.randn(6, 4), index=dates, columns=list("ABCD"))
...
def test_get_libraries_no_data_raises_exception(toplevel_tickstore, argus):
    date_range = DateRange(start=dt(2009, 1, 1), end=dt(2010, 12, 31, 23, 59, 59, 999000))
    with pytest.raises(NoDataFoundException):
        toplevel_tickstore._get_libraries(date_range)


def test_get_libraries_no_data_raises_exception_tzinfo_given(toplevel_tickstore, argus):
    tzinfo = mktz("Asia/Chongqing")
    date_range = DateRange(start=dt(2009, 1, 1, tzinfo=tzinfo), end=dt(2010, 12, 31, 23, 59, 59, 999000, tzinfo=tzinfo))
    with pytest.raises(NoDataFoundException):
        toplevel_tickstore._get_libraries(date_range)


def test_get_library_metadata(argus):
    argus.initialize_library("FEED_2010.LEVEL1", tickstore.TICK_STORE_TYPE)
    argus.initialize_library("FEED_2011.LEVEL1", tickstore.TICK_STORE_TYPE)
    argus.initialize_library("FEED.LEVEL1", toplevel.TICK_STORE_TYPE)
    toplevel_tickstore = argus["FEED.LEVEL1"]
    ...
    tzinfo = mktz("Asia/Chongqing")
    with pytest.raises(NoDataFoundException):
        toplevel_tickstore.read(
            symbol, DateRange(start=dt(2010, 1, 1, tzinfo=tzinfo), end=dt(2011, 1, 2, tzinfo=tzinfo)), columns=None
        )
