...
from argus.date import DateRange, mktz, CLOSED_CLOSED, CLOSED_OPEN, OPEN_CLOSED, OPEN_OPEN
...
def test_read_all_cols_all_dtypes(tickstore_lib, chunk_size):
    ...
    data[1]["os"] = None
    index = DatetimeIndex(
        [dt(1970, 1, 1, tzinfo=mktz("UTC")), dt(1970, 1, 1, 0, 0, 1, tzinfo=mktz("UTC"))],
    )
...
DUMMY_DATA = [
    ...
    {"b": 3.0, "c": 4.0, "index": dt(2013, 1, 2, tzinfo=mktz("Europe/London"))},
    {"b": 5.0, "c": 6.0, "index": dt(2013, 1, 3, tzinfo=mktz("Europe/London"))},
    {"b": 7.0, "c": 8.0, "index": dt(2013, 1, 4, tzinfo=mktz("Europe/London"))},
    {"b": 9.0, "c": 10.0, "index": dt(2013, 1, 5, tzinfo=mktz("Europe/London"))},
]
...
def test_date_range_default_timezone(tickstore_lib, tz_name):
    ...
    """
    DUMMY_DATA = [
        {"a": 1.0, "b": 2.0, "index": dt(2013, 1, 1, tzinfo=mktz(tz_name))},
        # Half-way through the year
        {"b": 3.0, "c": 4.0, "index": dt(2013, 7, 1, tzinfo=mktz(tz_name))},
    ]
...
def test_write_no_tz(tickstore_lib):
    DUMMY_DATA = [{"a": 1.0, "b": 2.0, "index": dt(2013, 6, 1, 12, 00)}]
    ...
    assert (
        len(
            tickstore_lib.read(
                "SYM",
                columns=None,
                date_range=DateRange(dt(2013, 6, 1, tzinfo=mktz("UTC")), dt(2013, 6, 1, 12, tzinfo=mktz("UTC"))),
            )
        )
        == 2
    )
...
def test_read_chunk_boundaries(tickstore_lib):
    SYM1_DATA = [
        {"a": 1.0, "b": 2.0, "index": dt(2013, 6, 1, 12, 00, tzinfo=mktz("UTC"))},
        {"a": 3.0, "b": 4.0, "index": dt(2013, 6, 1, 13, 00, tzinfo=mktz("UTC"))},
        # Chunk boundary here
        {"a": 5.0, "b": 6.0, "index": dt(2013, 6, 1, 14, 00, tzinfo=mktz("UTC"))},
    ]
    SYM2_DATA = [
        {"a": 7.0, "b": 8.0, "index": dt(2013, 6, 1, 12, 30, tzinfo=mktz("UTC"))},
        {"a": 9.0, "b": 10.0, "index": dt(2013, 6, 1, 13, 30, tzinfo=mktz("UTC"))},
        # Chunk boundary here
        {"a": 11.0, "b": 12.0, "index": dt(2013, 6, 1, 14, 30, tzinfo=mktz("UTC"))},
    ]
...
def test_read_spanning_chunks(tickstore_lib):
    SYM1_DATA = [
        {"a": 1.0, "b": 2.0, "index": dt(2013, 6, 1, 11, 00, tzinfo=mktz("UTC"))},
        {"a": 3.0, "b": 4.0, "index": dt(2013, 6, 1, 12, 00, tzinfo=mktz("UTC"))},
        # Chunk boundary here
        {"a": 5.0, "b": 6.0, "index": dt(2013, 6, 1, 14, 00, tzinfo=mktz("UTC"))},
    ]
    SYM2_DATA = [
    ...
    assert tickstore_lib._mongo_date_range_query(
        ["SYM1", "SYM2"],
        date_range=DateRange(dt(2013, 6, 1, 12, 45, tzinfo=mktz("UTC")), dt(2013, 6, 1, 15, 00, tzinfo=mktz("UTC"))),
    ) == {"s": {"$gte": dt(2013, 6, 1, 12, 30, tzinfo=mktz("UTC")), "$lte": dt(2013, 6, 1, 15, 0, tzinfo=mktz("UTC"))}}


def test_read_inside_range(tickstore_lib):
    SYM1_DATA = [
        {"a": 1.0, "b": 2.0, "index": dt(2013, 6, 1, 0, 00, tzinfo=mktz("UTC"))},
        {"a": 3.0, "b": 4.0, "index": dt(2013, 6, 1, 1, 00, tzinfo=mktz("UTC"))},
        # Chunk boundary here
        {"a": 5.0, "b": 6.0, "index": dt(2013, 6, 1, 14, 00, tzinfo=mktz("UTC"))},
    ]
    SYM2_DATA = [
    ...
    assert tickstore_lib._mongo_date_range_query(
        ["SYM1", "SYM2"],
        date_range=DateRange(dt(2013, 6, 1, 10, 0, tzinfo=mktz("UTC")), dt(2013, 6, 1, 15, 0, tzinfo=mktz("UTC"))),
    ) == {"s": {"$gte": dt(2013, 6, 1, 10, 0, tzinfo=mktz("UTC")), "$lte": dt(2013, 6, 1, 15, 0, tzinfo=mktz("UTC"))}}


def test_read_longs(tickstore_lib):
    DUMMY_DATA = [
        {"a": 1, "index": dt(2013, 6, 1, 12, 00, tzinfo=mktz("Europe/London"))},
        {"b": 4, "index": dt(2013, 6, 1, 13, 00, tzinfo=mktz("Europe/London"))},
    ]
    tickstore_lib._chunk_size = 3
    tickstore_lib.write("SYM", DUMMY_DATA)
    tickstore_lib.read("SYM", columns=None)
...
def test_read_with_image(tickstore_lib):
    DUMMY_DATA = [
        {"a": 1.0, "index": dt(2013, 1, 1, 11, 00, tzinfo=mktz("Europe/London"))},
        {"b": 4.0, "index": dt(2013, 1, 1, 12, 00, tzinfo=mktz("Europe/London"))},
    ]
    # Add an image
    tickstore_lib.write("SYM", DUMMY_DATA)
...
def test_read_unicode_strings(tickstore_lib):
    df = pd.DataFrame(
        data={"data": ["一", "二", "三"]},  # Chinese character [one, two , three]
        index=pd.Index(
            data=[
                dt(2016, 1, 1, 00, tzinfo=mktz("UTC")),
                dt(2016, 1, 2, 00, tzinfo=mktz("UTC")),
