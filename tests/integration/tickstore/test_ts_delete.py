...
from argus.date import DateRange, CLOSED_OPEN, mktz
...
def test_delete(tickstore_lib):
    ...
    deleted = tickstore_lib.delete(
        "SYM", DateRange(dt(2013, 1, 1, tzinfo=mktz("Europe/London")), dt(2013, 1, 2, tzinfo=mktz("Europe/London")))
    )
...
def test_delete_daterange(tickstore_lib):
    DUMMY_DATA = [
        {"a": 1.0, "b": 2.0, "index": dt(2013, 1, 1, tzinfo=mktz("Europe/London"))},
        {"a": 3.0, "b": 4.0, "index": dt(2013, 2, 1, tzinfo=mktz("Europe/London"))},
    ]
    tickstore_lib._chunk_size = 1
    tickstore_lib.write("SYM", DUMMY_DATA)
    ...
    deleted = tickstore_lib.delete(
        "SYM",
        DateRange(
            dt(2013, 1, 1, tzinfo=mktz("Europe/London")), dt(2013, 2, 1, tzinfo=mktz("Europe/London")), CLOSED_OPEN
        ),
    )
    assert deleted.deleted_count == 1
    df = tickstore_lib.read("SYM", columns=None)
