...
from argus._util import FwPointersCfg
from argus.store._ndarray_store import NdarrayStore, _APPEND_COUNT
...
def test_append_with_extra_columns(library):
    ...
    expected = np.array(
        ...
        dtype=np.dtype([("A", "S1"), ("B", np.int), ("C", np.float), ("D", "S1"), ("E", np.int), ("F", np.float)]),
    )
    library.write("MYARR", ndarr)
    library.append("MYARR", ndarr2)
    saved_arr = library.read("MYARR").data
