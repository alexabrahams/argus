...
from argus._compression import decompress
from argus.date import DateRange, mktz

# Do not remove PandasStore, used in global scope
from argus.store._pandas_ndarray_store import PandasDataFrameStore, PandasSeriesStore, PandasStore
from argus.store.version_store import register_versioned_storage
from tests.util import assert_frame_equal_

register_versioned_storage(PandasDataFrameStore)


def test_write_multi_column_to_argus_1_40_data(multicolumn_store_with_uncompressed_write):
...
def test_save_read_pandas_series_with_unicode_index_name(library):
    df = Series(
        ...
        index=MultiIndex.from_tuples(
            [(np.datetime64(dt(2013, 1, 1)),), (np.datetime64(dt(2013, 1, 2)),), (np.datetime64(dt(2013, 1, 3)),)],
            names=["DATETIME"],
        ),
    )
    library.write("pandas", df)
...
def test_save_read_pandas_dataframe_empty_multiindex_and_no_columns(library):
    expected = read_csv(StringIO("""STRATEGY MAC INSTRUMENT CONTRACT"""), delimiter=" ").set_index(
...
def test_dataframe_append_should_promote_string_column(library):
    ...
    df = DataFrame(
        data, index=DatetimeIndex(np.array([dt(2013, 1, 1), dt(2013, 1, 2)]).astype("datetime64[ns]"), name="DATETIME")
    )
    ...
    expected = DataFrame(
        expected_data,
        index=DatetimeIndex(
            np.array([dt(2013, 1, 1), dt(2013, 1, 2), dt(2013, 1, 3)]).astype("datetime64[ns]"), name="DATETIME"
        ),
    )
...
def test_save_read_ints(library):
    ts1 = DataFrame(
        index=[dt(2012, 1, 1) + dtd(hours=x) for x in range(5)], data={"col1": np.arange(5), "col2": np.arange(5)}
    )
...
def test_save_read_datetimes(library):
    ...
    ts1 = DataFrame(
        ...
        data={"field1": [1, 2, 3], "revision": [dt(2013, 1, 1), dt(2013, 1, 2), dt(2013, 1, 3)], "field2": [4, 5, 6]},
    )
    ts1.index.name = "index"
    library.write("TEST_1", ts1)
    ts2 = library.read("TEST_1").data
...
def test_daterange_end(library):
    ...
    with patch("argus.store._ndarray_store.decompress", mdecompressALL):
        library.read("MYARR").data
    mdecompressLR = Mock(side_effect=decompress)
    with patch("argus.store._ndarray_store.decompress", mdecompressLR):
...
@pytest.mark.parametrize(
    ...
    [
        (
            DataFrame(
                index=date_range(dt(2001, 1, 1), freq="D", periods=30000), data=list(range(30000)), columns=["A"]
            ),
            assert_frame_equal,
        ),
        (Series(index=date_range(dt(2001, 1, 1), freq="D", periods=30000), data=range(30000)), assert_series_equal),
    ],
)
def test_daterange(library, df, assert_equal):
    ...
    assert_equal(
        df[df.index[0] : df.index[0]], library.read("MYARR", date_range=DateRange(df.index[0], df.index[0])).data
    )
...
def test_daterange_append(library):
    ...
    assert_frame_equal_(
        df, library.read("MYARR", date_range=DateRange(df.index[0], df.index[-1])).data, check_freq=False
    )
    # append two more rows
...
def test_forced_encodings_with_df_mixed_types(library):
    ...
    # Argus currently converts all series names to bytes
    assert type(s_str_normal.name) == bytes
    assert type(s_unicode_normal.name) == bytes
    # Argus currently converts all column and index string type names to unicode
...
def test_forced_encodings_with_df(library):
    ...
    assert type(s_str_normal.name) == bytes
    assert type(s_unicode_normal.name) == bytes
    # Argus currently converts all column to unicode and will keep index type as is
    assert all([type(x) == unicode for x in df_normal.columns])
...
def test_forced_encodings_with_df_py3(library):
    ...
    # Argus currently converts all column to unicode_type and will keep index type as is
...
def test_forced_encodings_with_df_py3_multi_index(library):
    ...
    multi_index_df = pd.DataFrame(
        sample_data, index=pd.MultiIndex.from_tuples([(b"ele1", b"uni_type1"), (b"ele2", b"uni_type2")])
    )
