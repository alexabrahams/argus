...
import argus
from argus.store import _version_store_utils as vsu
...
def n_append(
    library, library_name, total_appends, rows_per_append, bulk_data_ts, start_idx, do_snapshots=True, do_prune=True
):
    open_last_row = 0
    for i in range(total_appends):
        ...
        library.append(
            symbol, bulk_data_ts[first_row:open_last_row], metadata={"snap": snap}, prune_previous_version=do_prune
        )
        if do_snapshots:
...
@pytest.mark.skip(
    reason="Not critical as upsert=False is rarely used. A more specific handling/exception is required here."
)
...
def test_append_without_corrupt_check(library, library_name):
    orig_check = argus.store._ndarray_store.CHECK_CORRUPTION_ON_APPEND
    argus.store._ndarray_store.set_corruption_check_on_append(False)
    try:
        with pytest.raises(OperationFailure):
            _corrupt_with_append_only(library, library_name)
    finally:
        argus.store._ndarray_store.set_corruption_check_on_append(orig_check)


def test_append_with_corrupt_check(library, library_name):
    orig_check = argus.store._ndarray_store.CHECK_CORRUPTION_ON_APPEND
    argus.store._ndarray_store.set_corruption_check_on_append(True)
    try:
        _corrupt_with_append_only(library, library_name)
    finally:
        argus.store._ndarray_store.set_corruption_check_on_append(orig_check)
