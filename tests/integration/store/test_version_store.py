...
import argus
from argus import VERSION_STORE, PandasDataFrameStore, PandasSeriesStore
from argus._config import FwPointersCfg, FW_POINTERS_REFS_KEY
from argus._util import mongo_count, get_fwptr_config
...
class FwPointersCtx:
    ...
    def __enter__(self):
        self.orig_value = argus.store._ndarray_store.ARGUS_FORWARD_POINTERS_CFG
        argus.store._ndarray_store.ARGUS_FORWARD_POINTERS_CFG = self.value_to_test

        self.reconcile_orig_value = argus.store._ndarray_store.ARGUS_FORWARD_POINTERS_RECONCILE
        argus.store._ndarray_store.ARGUS_FORWARD_POINTERS_RECONCILE = self.do_reconcile

    def __exit__(self, *args):
        argus.store._ndarray_store.ARGUS_FORWARD_POINTERS_CFG = self.orig_value
        argus.store._ndarray_store.ARGUS_FORWARD_POINTERS_RECONCILE = self.reconcile_orig_value
...
def test_append_corrupted_new_version(library, fw_pointers_cfg):
    with FwPointersCtx(fw_pointers_cfg):
        to_append = read_str_as_pandas(
            """  times | near
                          2012-11-09 17:06:11.040 |  30.0"""
        )
        to_append_2 = read_str_as_pandas(
            """  times | near
        ...
        assert library.read(symbol).data["near"][-1] == 40.0
...
def test_list_version_snapshot(library):
    ...
    assert set(x["symbol"] for x in library.list_versions()) == set(["A", "B", "C"])
...
def test_prunes_previous_version_append_interaction(library, fw_pointers_cfg):
    with FwPointersCtx(fw_pointers_cfg):
        ...
        ts3 = ts.append(
            pd.DataFrame(
                index=[ts2.index[-1] + dtd(days=1), ts2.index[-1] + dtd(days=2)], data=[4.8, 4.9], columns=["near"]
            )
        )
        ts3.index.name = ts1.index.name
        ts4 = ts
...
def _rnd_df(nrows, ncols):
    ret_df = pd.DataFrame(
        ...
        index=pd.date_range("20170101", periods=nrows, freq="S"),
...
def test_prune_previous_versions_retries_on_cleanup_error(library, fw_pointers_cfg):
    with FwPointersCtx(fw_pointers_cfg):
        ...
        with patch("argus.store.version_store.cleanup", side_effect=_cleanup) as cleanup:
            cleanup.__name__ = "cleanup"  # required by functools.wraps
            library._prune_previous_versions(symbol, keep_mins=0)

        assert len(list(library._argus_lib.get_top_level_collection().find({"symbol": symbol}))) == 1
...
def test_prune_previous_versions_retries_find_calls(library, fw_pointers_cfg):
    with FwPointersCtx(fw_pointers_cfg):
        ...
        def _next(*args, **kwargs):
            vs_caller_name = next(c for c in inspect.stack() if c[1].endswith("argus/store/version_store.py"))[3]
...
def test_append_does_not_duplicate_data_when_prune_fails(library, fw_pointers_cfg):
    with FwPointersCtx(fw_pointers_cfg):
        side_effect = [OperationFailure(0), argus.store.version_store.VersionStore._prune_previous_versions]
...
def test_write_does_not_succeed_with_a_prune_error(library, fw_pointers_cfg):
    with FwPointersCtx(fw_pointers_cfg):
        ...
        side_effect = [ValueError, argus.store.version_store.VersionStore._prune_previous_versions]
        library.write(symbol, ts1)
...
def test_empty_string_column_name(library):
    ...
    with pytest.raises(ArgusException):
        library.write("df", df)
...
def test_write_non_serializable_throws(argus):
    lib_name = "write_hanlder_test"
    argus.initialize_library(lib_name, VERSION_STORE)
    with patch("argus.store.version_store.STRICT_WRITE_HANDLER_MATCH", True):
...
@pytest.mark.parametrize("fw_pointers_cfg", [FwPointersCfg.DISABLED, FwPointersCfg.HYBRID, FwPointersCfg.ENABLED])
def test_write_non_serializable_pickling_default(argus, fw_pointers_cfg):
    with FwPointersCtx(fw_pointers_cfg):
        lib_name = "write_hanlder_test"
        argus.initialize_library(lib_name, VERSION_STORE)
        library = argus[lib_name]
...
@pytest.mark.parametrize("fw_pointers_cfg", [FwPointersCfg.DISABLED, FwPointersCfg.HYBRID, FwPointersCfg.ENABLED])
def test_write_strict_no_daterange(argus, fw_pointers_cfg):
    with FwPointersCtx(fw_pointers_cfg):
        lib_name = "write_hanlder_test"
        argus.initialize_library(lib_name, VERSION_STORE)
...
def test_handler_check_default_false(argus):
    lib_name = "write_hanlder_test1"
    argus.initialize_library(lib_name, VERSION_STORE)
    assert argus[lib_name]._with_strict_handler_match is False


def test_handler_check_default_osenviron(argus):
    with patch("argus.store.version_store.STRICT_WRITE_HANDLER_MATCH", True):
        lib_name = "write_hanlder_test2"
        argus.initialize_library(lib_name, VERSION_STORE)
        assert argus[lib_name]._with_strict_handler_match is True


def test_handler_check_set_false(argus):
    lib_name = "write_hanlder_test3"
    argus.initialize_library(lib_name, VERSION_STORE, strict_write_handler=False)
    assert argus[lib_name]._with_strict_handler_match is False


def test_handler_check_set_true(argus):
    lib_name = "write_hanlder_test4"
    argus.initialize_library(lib_name, VERSION_STORE, strict_write_handler=True)
    assert argus[lib_name]._with_strict_handler_match is True
...
@pytest.mark.parametrize(
    "input_series",
    [
        _mixed_test_data()["with_some_objects"][0]["n1"],
...
def test_can_write_tz_aware_data_df(library, fw_pointers_cfg):
    with FwPointersCtx(fw_pointers_cfg):
        ...
        # Argus converts by default the data to UTC, convert back
...
def test_fwpointers_mixed_scenarios(library, write_cfg, read_cfg, append_cfg, reread_cfg):
    ...
    orig_check_written = argus.store._ndarray_store.NdarrayStore.check_written
    outer = {
        "round": -1,  # unfortunately "nonlocal" keyword is not available in Python 2, this is a workaround
        "raised": False,
    }
    ...
    with patch("argus.store._ndarray_store.NdarrayStore.check_written") as mock_check_written:
        ...
        argus.store._ndarray_store.NdarrayStore.check_written = _mock_check_written
...
def test_version_argus_version(argus):
    import argus.store.version_store as vs

    orig_val = vs.ARGUS_VERSION_NUMERICAL
    try:
        lib_name = "argus_version_test"
        argus.initialize_library(lib_name, VERSION_STORE)
        library = argus[lib_name]
        for i in range(3):
        ...
        vs.ARGUS_VERSION_NUMERICAL = orig_val
...
@pytest.mark.parametrize(
    "first_write_cfg, second_write_cfg",
    [
