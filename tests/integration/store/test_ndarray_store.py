...
from argus.store._ndarray_store import NdarrayStore
...
def test_write_new_column_name_to_argus_1_40_data(ndarray_store_with_uncompressed_write):
...
def test_save_and_resave_reuses_chunks(library, fw_pointers_cfg):
    with FwPointersCtx(fw_pointers_cfg):
        with patch("argus.store._ndarray_store._CHUNK_SIZE", 1000):
