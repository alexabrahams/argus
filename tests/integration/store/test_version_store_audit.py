...
from argus.exceptions import ConcurrentModificationException
from argus.store.audit import ArgusTransaction
...
def test_ArgusTransaction_can_do_first_writes(library):
    with ArgusTransaction(library, "SYMBOL_NOT_HERE", "user", "log") as cwb:
        cwb.write("SYMBOL_NOT_HERE", ts1)
    wrote_vi = library.read("SYMBOL_NOT_HERE")
    assert_frame_equal(wrote_vi.data, ts1)


def test_ArgusTransaction_detects_concurrent_writes(library):
    ...
    def losing_writer():
        ...
        with pytest.raises(ConcurrentModificationException):
            with ArgusTransaction(library, "FOO", "user", "log") as cwb:
...
def test_audit_writes(library):
    with ArgusTransaction(library, symbol, "u1", "l1") as mt:
        mt.write(symbol, ts1)

    with ArgusTransaction(library, symbol, "u2", "l2") as mt:
        mt.write(symbol, ts2)
...
def test_audit_read(library):
    with ArgusTransaction(library, symbol3, "u3", "foo") as mt:
    ...
    with ArgusTransaction(library, symbol2, "u2", "l2") as mt:
...
def test_corrupted_read_writes_new(library):
    ...
    with ArgusTransaction(library, symbol, "u1", "l2") as mt:
        mt.write(symbol, ts2)
...
def test_write_after_delete(library):
    with ArgusTransaction(library, symbol, "u1", "l") as mt:
        mt.write(symbol, ts1)
...
def test_ArgusTransaction_write_skips_for_exact_match(library):
...
def test_ArgusTransaction_write_doesnt_skip_for_close_ts(library):
    orig_ts = read_str_as_pandas(
        """times |    PX_LAST
                  2014-10-31 21:30:00.000 | 204324.674
                  2014-11-13 21:30:00.000 |  193964.45
                  2014-11-14 21:30:00.000 | 193650.403"""
