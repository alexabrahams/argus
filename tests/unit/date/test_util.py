...
from argus.date._util import to_dt, utc_dt_to_local_dt
...
@patch("argus.date._util.mktz", lambda zone="Asia/Shanghai": mktz(zone))
def test_utc_dt_to_local_dt():
    with pytest.raises(ValueError):
        assert utc_dt_to_local_dt(dt(2000, 1, 1, 0, 0, 0, tzinfo=mktz("Asia/Shanghai")))
