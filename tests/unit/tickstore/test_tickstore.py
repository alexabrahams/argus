...
from argus.date import CLOSED_OPEN
from argus.date._daterange import DateRange
from argus.date._mktz import mktz
from argus.exceptions import UnorderedDataException
from argus.tickstore.tickstore import (
    TickStore,
    IMAGE_DOC,
    IMAGE,
    START,
...
def test_mongo_date_range_query():
    ...
    self._collection.aggregate.return_value = iter(
        [
            {"_id": "s1", "start": dt(2014, 1, 1, 0, 0, tzinfo=mktz())},
...
def test_strify_tickstore():
    ...
    self = create_autospec(TickStore)
    self._argus_lib = sentinel.library
...
def test_tickstore_to_bucket_no_image():
    symbol = "SYM"
    data = [
        {"index": dt(2014, 1, 1, 0, 1, tzinfo=mktz()), "A": 124, "D": 0},
