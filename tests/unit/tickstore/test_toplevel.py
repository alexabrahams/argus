...
from argus.exceptions import OverlappingDataException
from argus.exceptions import UnhandledDtypeException
from argus.tickstore.tickstore import TickStore
from argus.tickstore.toplevel import TopLevelTickStore, TickStoreLibrary
...
def test_raise_exception_and_log_an_error_if_an_invalid_library_name_is_added():
    argus_lib = MagicMock()
    argus_lib.argus.__getitem__.side_effect = Exception()
    store = TopLevelTickStore(argus_lib)
    with patch("argus.tickstore.toplevel.logger") as mock_logger:
...
def test_raise_exception_if_date_range_overlaps():
    self = create_autospec(TopLevelTickStore, _argus_lib=MagicMock())
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            dt(2010, 1, 1),
            dt(2010, 12, 31, 23, 59, 59, 999000),
            dt(2010, 1, 1, tzinfo=mktz("UTC")),
            dt(2010, 12, 31, 23, 59, 59, 999000, tzinfo=mktz("UTC")),
        ),
        (
            dt(2009, 12, 31, 19, tzinfo=mktz("America/New_York")),
            dt(2010, 12, 31, 18, 59, 59, 999000, tzinfo=mktz("America/New_York")),
            dt(2010, 1, 1, tzinfo=mktz("UTC")),
            dt(2010, 12, 31, 23, 59, 59, 999000, tzinfo=mktz("UTC")),
        ),
    ],
...
def test_add_library_to_colllection_if_date_range_is_on_UTC_or_naive_day_boundaries(
    ...
    self = create_autospec(TopLevelTickStore, _argus_lib=MagicMock(), _collection=MagicMock())
    self._get_library_metadata.return_value = []
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            dt(2009, 12, 31, 21, 10, tzinfo=mktz("America/New_York")),
            dt(2010, 12, 31, 9, 21, tzinfo=mktz("America/New_York")),
        ),
    ],
)
...
def test_write_pandas_data_to_right_libraries():
    ...
    mock_lib1 = Mock()
    mock_lib2 = Mock()
    when(self._argus_lib.argus.__getitem__).called_with(sentinel.libname1).then(mock_lib1)
    when(self._argus_lib.argus.__getitem__).called_with(sentinel.libname2).then(mock_lib2)
    with patch("argus.tickstore.toplevel.to_dt") as patch_to_dt:
