...
from argus.exceptions import ConcurrentModificationException, NoDataFoundException
from argus.store.audit import ArgusTransaction, DataChange
from argus.store.version_store import VersionedItem, VersionStore
...
def test_ArgusTransaction_simple():
    vs = Mock(spec=VersionStore)
    ts1 = pd.DataFrame(index=[1, 2], data={"a": [1.0, 2.0]})
    vs.read.return_value = VersionedItem(
        symbol=sentinel.symbol, library=sentinel.library, version=1, metadata=None, data=ts1, host=sentinel.host
    )
    vs.write.return_value = VersionedItem(
        symbol=sentinel.symbol, library=sentinel.library, version=2, metadata=None, data=None, host=sentinel.host
    )
    vs.list_versions.return_value = [{"version": 2}, {"version": 1}]

    with ArgusTransaction(vs, sentinel.symbol, sentinel.user, sentinel.log) as cwb:
...
def test_ArticTransaction_no_audit():
    ...
    with ArgusTransaction(vs, sentinel.symbol, sentinel.user, sentinel.log, audit=False) as cwb:
        cwb.write(sentinel.symbol, pd.DataFrame(index=[3, 4], data={"a": [1.0, 2.0]}), metadata=sentinel.meta)
...
def test_ArgusTransaction_writes_if_metadata_changed():
    vs = Mock(spec=VersionStore)
    ts1 = pd.DataFrame(index=[1, 2], data={"a": [1.0, 2.0]})
    vs.read.return_value = VersionedItem(
        symbol=sentinel.symbol, library=sentinel.library, version=1, metadata=None, data=ts1, host=sentinel.host
    ...
    vs.read.return_value = VersionedItem(
        symbol=sentinel.symbol, library=sentinel.library, version=2, metadata={1: 2}, data=ts1, host=sentinel.host
    )
    with ArgusTransaction(vs, sentinel.symbol, sentinel.user, sentinel.log) as cwb:
        assert cwb._do_write is False
        cwb.write(sentinel.symbol, ts1, metadata={1: 2})
...
def test_ArgusTransaction_writes_if_base_data_corrupted():
    ...
    vs.read_metadata.return_value = VersionedItem(
        symbol=sentinel.symbol, library=sentinel.library, version=1, metadata=None, data=None, host=sentinel.host
    )
    vs.list_versions.return_value = [{"version": 2}, {"version": 1}]
...
def test_ArgusTransaction_writes_no_data_found():
    vs = Mock(spec=VersionStore)
    ts1 = pd.DataFrame(index=[1, 2], data={"a": [1.0, 2.0]})
...
def test_ArgusTransaction_writes_no_data_found_deleted():
    vs = Mock(spec=VersionStore)
    ts1 = pd.DataFrame(index=[1, 2], data={"a": [1.0, 2.0]})
    vs.read.side_effect = NoDataFoundException("no data")
    vs.write.return_value = VersionedItem(
        symbol=sentinel.symbol, library=sentinel.library, version=3, metadata=None, data=None, host=sentinel.host
    )
    vs.list_versions.side_effect = [
        [{"version": 2}, {"version": 1}],
        [{"version": 3}, {"version": 2}],
...
def test_ArgusTransaction_does_nothing_when_data_not_modified():
    vs = Mock(spec=VersionStore)
    ts1 = pd.DataFrame(index=[1, 2], data={"a": [1.0, 2.0]})
    vs.read.return_value = VersionedItem(
        symbol=sentinel.symbol, library=sentinel.library, version=1, metadata=None, data=ts1, host=sentinel.host
...
def test_ArgusTransaction_does_nothing_when_data_is_None():
    vs = Mock(spec=VersionStore)
    ts1 = pd.DataFrame(index=[1, 2], data={"a": [1.0, 2.0]})
    vs.read.return_value = VersionedItem(
        symbol=sentinel.symbol, library=sentinel.library, version=1, metadata=None, data=ts1, host=sentinel.host
...
def test_ArgusTransaction_guards_against_inconsistent_ts():
    vs = Mock(spec=VersionStore)
    ts1 = pd.DataFrame(index=[1, 2], data={"a": [1.0, 2.0]})
    vs.read.return_value = VersionedItem(
        symbol=sentinel.symbol, library=sentinel.library, version=1, metadata=None, data=ts1, host=sentinel.host
    ...
    with pytest.raises(ConcurrentModificationException):
        with ArgusTransaction(vs, sentinel.symbol, sentinel.user, sentinel.log, modify_timeseries=ts1) as cwb:
            pass


# TODO: Skipping this flaky test for now.
def _test_ArgusTransaction_detects_concurrent_writes():
    vs = Mock(spec=VersionStore)
    ts1 = pd.DataFrame(index=[1, 2], data={"a": [1.0, 2.0]})
    vs.read.return_value = VersionedItem(
        symbol=sentinel.symbol, library=sentinel.library, version=1, metadata=None, data=ts1, host=sentinel.host
    ...
    def losing_writer():
        ...
        with pytest.raises(ArgusTransaction):
            with ArgusTransaction(vs, sentinel.symbol, sentinel.user, sentinel.log) as cwb:
