...
from argus.store.versioned_item import VersionedItem


def test_versioned_item_str():
    item = VersionedItem(
        symbol="sym", library="ONEMINUTE", data=pd.DataFrame(), version=1.0, host="myhost", metadata={"metadata": "foo"}
    )
...
def test_versioned_item_default_host():
    ...
    expected_item = VersionedItem(
        symbol="sym", library="ONEMINUTE", data=[1, 2, 3], version=1.0, host=None, metadata={"metadata": "foo"}
    )
