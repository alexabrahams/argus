...
from argus.store._pandas_ndarray_store import PandasDataFrameStore, PandasPanelStore, PandasStore
...
def test_panel_converted_to_dataframe_and_stacked_to_write():
    ...
    with patch.object(PandasDataFrameStore, "write") as mock_write:
        with patch("argus.store._pandas_ndarray_store.DataFrame") as DF:
...
def test_read_multi_index_with_no_ts_info():
    ...
    df = read_str_as_pandas(
        """index 1 |    index 2 | SPAM
                            2012-09-08 | 2015-01-01 |  1.0
                            2012-09-09 | 2015-01-02 |  1.1
                            2012-10-08 | 2015-01-03 |  2.0""",
        num_index=2,
    )
    ...
    record = np.array(
        record.tolist(),
        dtype=np.dtype(
            [("index 1", "<M8[ns]"), ("index 2", "<M8[ns]"), ("SPAM", "<f8")],
