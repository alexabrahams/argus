...
from argus._compression import compress, compressHC
from argus.exceptions import UnsupportedPickleStoreVersion
from argus.store._pickle_store import PickleStore
from argus.store._version_store_utils import checksum
...
def test_write():
    ...
    version = {}
    PickleStore.write(self, sentinel.argus_lib, version, sentinel.symbol, "item", sentinel.previous_version)
...
def test_write_object():
    ...
    PickleStore.write(self, argus_lib, version, "sentinel.symbol", sentinel.item, sentinel.previous_version)
    assert "data" not in version

    assert version["blob"] == "__chunked__V2"
    coll = argus_lib.get_top_level_collection.return_value
    assert coll.update_one.call_args_list == [
        call(
            {
                "sha": checksum(
                    "sentinel.symbol",
                    {"segment": 0, "data": Binary(compress(cPickle.dumps(sentinel.item, cPickle.HIGHEST_PROTOCOL)))},
                ),
...
def test_read():
    self = create_autospec(PickleStore)
    version = {"data": "item"}
    assert PickleStore.read(self, sentinel.argus_lib, version, sentinel.symbol) == "item"


def test_read_object_backwards_compat():
    self = create_autospec(PickleStore)
    version = {"blob": Binary(compressHC(cPickle.dumps(object)))}
    assert PickleStore.read(self, sentinel.argus_lib, version, sentinel.symbol) == object
...
def test_read_object_2():
    ...
    argus_lib.get_top_level_collection.return_value = coll

    assert PickleStore.read(self, argus_lib, version, sentinel.symbol) == object
...
def test_read_backward_compatibility():
    ...
    df = store.read(sentinel.argus_lib, version, sentinel.symbol)
...
def test_unpickle_highest_protocol():
    ...
    store = PickleStore()
    ps = store.read(sentinel.argus_lib, version, sentinel.symbol)
...
def test_pickle_chunk_V1_read():
    ...
    ps = PickleStore()
    assert data == ps.read(argus_lib, version, sentinel.symbol)
...
def test_pickle_store_future_version():
    ...
    with pytest.raises(UnsupportedPickleStoreVersion) as e:
        ps.read(argus_lib, version, sentinel.symbol)
