...
from argus.argus import ArgusLibraryBinding, Argus
from argus.date import mktz
from argus.exceptions import DuplicateSnapshotException, NoDataFoundException
from argus.store import version_store
from argus.store.version_store import VersionStore, VersionedItem


def test_delete_version_version_not_found():
    with patch("argus.store.version_store.VersionStore.__init__", return_value=None, autospec=True):
        with patch("argus.store.version_store.logger") as logger:
...
def test_list_versions_no_snapshot():
    ...
    date = dt(2013, 4, 1, 9, 0)
    vs._versions.find.return_value = [
        {"_id": bson.ObjectId.from_datetime(date), "symbol": "s", "version": 10, "metadata": None, "parent": []}
    ]
    version = list(VersionStore.list_versions(vs, "symbol"))[0]
    local_date = date.replace(tzinfo=mktz("UTC"))
    assert version == {
...
def test_get_version_allow_secondary_True():
    ...
    assert vs._do_read.call_args_list == [
        call("symbol", vs._read_metadata.return_value, None, date_range=None, read_preference=sentinel.read_preference)
    ]
...
def test_read_metadata_no_asof():
    ...
    assert versions.find_one.call_args_list == [
        call({"symbol": sentinel.symbol}, sort=[("version", pymongo.DESCENDING)])
    ]
...
def test_write_check_quota():
    ...
    vs = create_autospec(
        ...
        _argus_lib=create_autospec(ArgusLibraryBinding, argus=create_autospec(Argus, mongo_host="some_host")),
    )
    ...
    assert vs._argus_lib.check_quota.call_count == 1


def test_initialize_library():
    argus_lib = create_autospec(ArgusLibraryBinding)
    argus_lib.argus = create_autospec(Argus, _allow_secondary=False)
    with patch("argus.store.version_store.enable_sharding", autospec=True) as enable_sharding:
        ...
        argus_lib.get_top_level_collection.__name__ = "get_top_level_collection"
        argus_lib.get_top_level_collection.return_value.database.list_collection_names.__name__ = (
            "list_collection_names"
        )
        VersionStore.initialize_library(argus_lib, hashed=sentinel.hashed)
    assert enable_sharding.call_args_list == [call(argus_lib.argus, argus_lib.get_name(), hashed=sentinel.hashed)]


def test_ensure_index():
    th = Mock()
    vs = create_autospec(VersionStore, _collection=Mock())
    with patch("argus.store.version_store._TYPE_HANDLERS", [th]):
...
def test_prune_previous_versions_0_timeout():
    ...
    with patch("argus.store.version_store.dt") as dt:
...
def test_read_handles_operation_failure():
    ...
    assert self._do_read.call_args_list == [
        call(
            sentinel.symbol,
            sentinel.meta1,
    ...
    assert self._do_read_retry.call_args_list == [
        call(
            sentinel.symbol,
            sentinel.meta2,
            sentinel.from_version,
...
def test_read_reports_random_errors():
    self = create_autospec(VersionStore, _versions=Mock(), _argus_lib=Mock(), _allow_secondary=True)
    self._collection = create_autospec(Collection)
    self._do_read.__name__ = "name"  # feh: mongo_retry decorator cares about this
    self._do_read.side_effect = Exception("bad")
    with pytest.raises(Exception) as e:
        with patch("argus.store.version_store.log_exception") as le:
...
def test_snapshot():
    ...
    ] or vs._read_metadata.call_args_list == [
        call("bar", as_of=None, read_preference=ReadPreference.PRIMARY),
        call("foo", as_of=None, read_preference=ReadPreference.PRIMARY),
    ]
...
TPL_VERSION = {
    "_id": ObjectId("5a2ffdf817f7041a4ff1aa82"),
    "base_version_id": ObjectId("5a2ffd5917f70412ca78d80a"),
    "append_count": 0,
    "dtype_metadata": {"index": ["index"], "columns": ["A", "B", "C", "D"]},
    "segment_count": 1,
    "symbol": "SYM_E",
    "up_to": 3,
    "metadata": None,
...
def _create_mock_versionstore():
    vs = create_autospec(VersionStore, _argus_lib=Mock(), _version_nums=Mock(), _versions=Mock())
    vs._insert_version = lambda version: VersionStore._insert_version(vs, version)
    vs._argus_lib.get_name.return_value = TEST_LIB
    ...
    vs.write.return_value = VersionedItem(
        symbol=TEST_SYMBOL,
        library=vs._argus_lib.get_name(),
        version=TPL_VERSION["version"] + 1,
        metadata=META_TO_WRITE,
        data=None,
        host=vs._argus_lib.argus.mongo_host,
    )
    return vs
...
def test_write_metadata_no_previous_data():
    ...
    assert vs.write.call_args_list == [
        call(TEST_SYMBOL, data=None, metadata=META_TO_WRITE, prune_previous_version=True, my_custom_arg="hello")
    ]
...
def test_write_metadata_with_previous_data():
    ...
    with patch("argus.store.version_store.bson.ObjectId") as mock_objId, patch(
        "argus.store.version_store.mongo_retry"
    ) as mock_retry:
...
def test_restore_version_data_missing_symbol():
    ...
    with patch("argus.store.version_store.mongo_retry") as mock_retry:
        mock_retry.side_effect = lambda f: f
...
def test_append_error_clean_retry():
    ...
    vs = create_autospec(
        ...
        _versions=Mock(
            insert_one=Mock(__name__="insert_one"), find_one=Mock(__name__="find_one", return_value=previous_version)
        ),
        _argus_lib=create_autospec(ArgusLibraryBinding, argus=create_autospec(Argus, mongo_host="some_host")),
    )
    vs._insert_version = lambda version: VersionStore._insert_version(vs, version)
