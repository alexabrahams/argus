...
from argus._compression import compress, compress_array, decompress, decompress_array, enable_parallel_lz4
...
def test_compress_LZ4():
    cfn = Mock()
    with patch("argus._compression.lz4_compress", cfn):
        compress(b"foo")
...
def test_compression_equal_regardless_parallel_mode():
    a = [b"spam "] * 666
    with patch("argus._compression.ENABLE_PARALLEL", True):
        parallel = compress_array(a)
    with patch("argus._compression.ENABLE_PARALLEL", False):
...
def test_enable_parallel_lz4():
    ...
    from argus._compression import ENABLE_PARALLEL
