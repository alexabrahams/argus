...
from argus.scripts.argus_create_user import main
from ...util import run_as_main


def test_main_minimal():
    with patch("argus.scripts.argus_create_user.logger", autospec=True) as logger, patch(
        "argus.scripts.argus_create_user.MongoClient", autospec=True
    ) as MC, patch("argus.scripts.argus_create_user.get_mongodb_uri", autospec=True) as get_mongodb_uri, patch(
        "argus.scripts.argus_create_user.do_db_auth", autospec=True
    ) as do_db_auth:
    ...
    assert MC.return_value.__getitem__.call_args_list == [call("argus_user")]
    db = MC.return_value.__getitem__.return_value
    assert [call("user", ANY, read_only=False)] == db.add_user.call_args_list
    assert logger.info.call_args_list == [
        call("Granted: user [WRITE] to argus_user"),
        call("User creds: argus_user/user/asdf"),
    ]


def test_main_with_db():
    with patch("argus.scripts.argus_create_user.MongoClient", autospec=True) as MC, patch(
        "argus.scripts.argus_create_user.get_mongodb_uri", autospec=True
    ) as get_mongodb_uri, patch("argus.scripts.argus_create_user.do_db_auth", autospec=True) as do_db_auth:
...
def test_no_auth():
    with patch("argus.scripts.argus_create_user.logger", autospec=True) as logger, patch(
        ...
        "argus.scripts.argus_create_user.do_db_auth", autospec=True, return_value=False
    ) as do_db_auth:
