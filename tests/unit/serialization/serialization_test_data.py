...
from argus.serialization.numpy_records import DataFrameSerializer
...
def _mixed_test_data():
    ...
    if _TEST_DATA is None:
        ...
        large_with_some_objects = create_test_data(
            size=10000, cols=64, index=True, multiindex=False, random_data=True, random_ids=True, use_hours=True
        )
        ...
        mixed_dtypes_df = pd.DataFrame(
            {
                ...
                "complex": np.array([1.0 + 4.0j, 2.0 + 5.0j, 3.0 + 6.0j]),
            }
        )
