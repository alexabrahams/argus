...
from argus.exceptions import ArgusSerializationException
from argus.serialization.incremental import IncrementalPandasToRecArraySerializer
...
def test_incremental_bad_init():
    with pytest.raises(ArgusSerializationException):
    ...
    with pytest.raises(ArgusSerializationException):
        IncrementalPandasToRecArraySerializer(
            df_serializer, _mixed_test_data()["small"][0], chunk_size=_CHUNK_SIZE, string_max_len=-1
        )
