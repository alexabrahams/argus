    ...
    def __init__(self, hostname, port, run_cmd, run_stdin=None, env=None, cwd=None):
        ...
        if "DEBUG" in os.environ:
            self.p = subprocess.Popen(
                self.run_cmd, env=self.env, cwd=self.cwd, stdin=subprocess.PIPE if run_stdin else None
            )
        else:
            self.p = subprocess.Popen(
                self.run_cmd,
...
class TestServer(Workspace):
    ...
    def get_port(self):
        ...
        if not self.random_port:
            return self.port_seed - int(
                hashlib.sha1((os.environ["USER"] + self.__class__.__name__).encode("utf-8")).hexdigest()[:3], 16
            )
    ...
    def wait_for_go(self, start_interval=0.1, retries_per_interval=3, retry_limit=28, base=2.0):
        ...
        while retry_count > 0:
            for _ in range(retries_per_interval):
                log.debug(
                    "sleeping for %s before retrying (%d of %d)"
